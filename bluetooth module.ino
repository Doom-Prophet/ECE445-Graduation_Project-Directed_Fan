...
void loop()
{
  ...
  if(byte_count)//If there are any bytes then deal with them
  {
    ...
    if(String(inData)=="on")//This could be any motor start string we choose from the python script
    {
      serial_connection.println("********* Start FAN *********");
    }
    else if(String(inData)=="off")//Again this is an arbitrary choice. It would probably be something like: MOTOR_STOP
    {
      serial_connection.println("********* STOP FAN *********");
    }
    else if(String(inData)=="up")//Again this is an arbitrary choice. It would probably be something like: MOTOR_STOP
    {
      serial_connection.println("********* SPEED UP *********");
    }
    else if(String(inData)=="down")//Again this is an arbitrary choice. It would probably be something like: MOTOR_STOP
    {
      serial_connection.println("********* SLOW DOWN *********");
    }
